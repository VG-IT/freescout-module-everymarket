...
class EverymarketServiceProvider extends ServiceProvider
{
    ...
    public function hooks()
    {
        ...
        // FreeScout's own outgoing emails may come back to the INBOX (Auto-BCC,
        // forwarding/routing rules) and get imported as duplicate threads,
        // because outgoing threads do not store their generated Message-ID in
        // threads.message_id. Skip fetched messages whose own Message-ID was
        // generated by this FreeScout instance for an existing thread.
        \Eventy::addFilter('fetch_emails.should_save_thread', function ($save, $data) {
            ...
            try {
                ...
                // Replies and auto replies: FS_reply-{thread_id}-{hash}@...
                $prefixes = [
                    \MailHelper::MESSAGE_ID_PREFIX_REPLY_TO_CUSTOMER,
                    \MailHelper::MESSAGE_ID_PREFIX_AUTO_REPLY,
                ];
                foreach ($prefixes as $prefix) {
                    if (preg_match('/^'.preg_quote($prefix, '/').'\-(\d+)\-([a-z0-9]+)@/', $message_id, $m)
                        && $m[2] == \MailHelper::getMessageIdHash($m[1])
                        && \App\Thread::find($m[1])
                    ) {
                        \Log::info('[Everymarket] Skipping own outgoing message fetched back: '.$message_id);
                        return false;
                    }
                }

                // User notifications: FS_notify-{thread_id}-{user_id}-...
                if (preg_match('/^'.preg_quote(\MailHelper::MESSAGE_ID_PREFIX_NOTIFICATION, '/').'\-(\d+)\-(\d+)\-/', $message_id, $m)
                    && \App\Thread::find($m[1])
                ) {
                    \Log::info('[Everymarket] Skipping own notification fetched back: '.$message_id);
                    return false;
                }
            } catch (\Throwable $e) {
    ...
    public function register()
    {
        ...
        $this->commands([
            ...
            \Modules\Everymarket\Console\CleanupDuplicateThreads::class,
        ]);
    }
